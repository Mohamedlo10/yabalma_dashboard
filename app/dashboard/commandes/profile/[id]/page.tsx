"use client";
import { getcommandesById, modifierCommande } from "@/app/api/commandes/query";
import { getSupabaseSession } from "@/lib/authMnager";
import BeatLoader from "react-spinners/BeatLoader";
import { Commande } from "../../schema";
import { supprimerCommande } from "@/app/api/commandes/query";
...
export default function Page({ params }: PageProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      setIsLoading(true);
      try {
        const data: any = await getcommandesById(parseInt(id));
        if (data != null) {
          console.log(data);
          setcommande(data);
          setActucommande({
            ...
            cancelled_status: data.cancelled_status || false,
          });
        }

        const session = getSupabaseSession();
        if (session != null) {
          if (session.access_groups?.commandes) {
   ...
   const [Actucommande, setActucommande] = useState({
     ...commande,
   ...
   const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
     ...
     setActucommande({ ...Actucommande, [name]: value });
   };
