...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const response = await modifierCommande(Actucommande.id, commandeSansClientEtAnnonce);    
  console.log("Reussi")
  seteditMode(false)
  ...
  return <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-36 rounded-lg">
            ...
            <div className="mb-4">
              <div className="text-gray-500 text-sm sm:text-base">Paiement</div>
              <select
            ...
            <div className="p-4">
            <div className="mb-4">
              <div className="text-gray-500 text-sm sm:text-base">Annuler</div>
              <select
                name="cancelled_status"
          ...
          <div className="grid grid-cols-2 gap-12">
            ...
            <div className='mb-6'>      
            <div className="grid grid-cols-3 gap-16 rounded-lg">
            {/* <!-- First Bloc --> */}
            <div className="p-4">
              {Actucommande.annonce ?(
                ...
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.annonce.source} {Actucommande?.annonce.sourceAddress} le {format(new Date(Actucommande?.annonce.date_depart), 'dd/MM/yyyy', { locale: fr })}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Destination</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.annonce.destination} {Actucommande?.annonce.destinationAddress} le {format(new Date(Actucommande?.annonce.date_arrive), 'dd/MM/yyyy', { locale: fr })}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Type de Transport</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.annonce.type_transport}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Date de publication</div>
               {commande? ( <div className="leading-6 mt-1 text-sm sm:text-base">{format(new Date(commande?.created_at), 'dd MMMM yyyy', { locale: fr })}
                {` à ${format(new Date(commande?.created_at), 'HH:mm')}`}</div>):
                (
            ...
            <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Annuler</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande.cancelled_status ? "Oui" : "Non"} </div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">État de paiement</div>
                <div
                  className={`leading-6 mt-1 text-sm sm:text-base px-2 py-1 rounded-md text-white 
                    ${Actucommande?.payment_status === 'unpaid' ? 'bg-red-500' : 'bg-green-500'}`}
                >
                  {Actucommande?.payment_status === 'unpaid' ? 'Non réglé' : 'Réglé'}
                </div>
              </div>

              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Lieu de depot</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.annonce?.lieu_depot}</div>
              </div>
             
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Statut</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.statut}</div>
              </div>
             ...
             <div className="p-4">
              {Actucommande.detail_commande?(
                <>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Type de la commande</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.detail_commande?.type || 'N/A'}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Nombre d'article</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.detail_commande?.articles?.length==0?'N/A':Actucommande?.detail_commande?.articles?.length}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Poids</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.detail_commande?.poids || 'N/A'}</div>
              </div>
              <div className="mb-4">
                <div className="text-gray-500 text-sm sm:text-base">Numero du destinataire</div>
                <div className="leading-6 mt-1 text-sm sm:text-base">{Actucommande?.detail_commande?.destinataire_number || 'N/A'}</div>
              </div>
