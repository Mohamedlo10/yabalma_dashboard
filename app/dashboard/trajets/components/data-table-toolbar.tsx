...
export function DataTableToolbar<TData>({
  ...
  const deviseFilter = table
    .getColumn("devise_prix")
  ...
  const handleWeightFilter = () => {
    if (weightFilter.minWeight || weightFilter.maxWeight) {
      ...
      table.getColumn("stock_annonce")?.setFilterValue({
        min: weightFilter.minWeight
          ? parseFloat(weightFilter.minWeight)
          : undefined,
        max: weightFilter.maxWeight
          ? parseFloat(weightFilter.maxWeight)
          : undefined,
      });
    }
  };
  ...
  const getStatusCount = (status: string) => {
    return table
      .getFilteredRowModel()
      .rows.filter((row) => row.getValue("statut") === status).length;
  };

  const totalRows = table.getFilteredRowModel().rows.length;
  const boostedCount = table
    .getFilteredRowModel()
    .rows.filter((row) => row.getValue("is_boost") === true).length;
        ...
        <div className="flex flex-1 items-center space-x-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-48">
              ...
              <DropdownMenuItem
                onClick={() =>
                  table.getColumn("type_transport")?.setFilterValue("economy")
                }
              ...
              <DropdownMenuItem
                onClick={() =>
                  table.getColumn("type_transport")?.setFilterValue("express")
                }
                className="flex items-center gap-2"
              >
                <div className="w-3 h-3 rounded-full bg-red-600"></div>
          ...
          {statusOptions.map((status) => (
            <Button
              ...
              className={`h-8 px-3 transition-all duration-200 ${
                statusFilter === status.value ? status.color : "hover:bg-muted"
              }`}
            >
          ...
          {table.getState().columnFilters.map((filter) => (
            <Badge key={filter.id} variant="outline" className="text-xs">
              {filter.id === "statut" && filter.value
                ? `Statut: ${
                    statusOptions.find((s) => s.value === filter.value)?.label
                  }`
                ...
                ? `Devise: ${
                    deviseOptions.find((d) => d.value === filter.value)?.label
                  }`
