...
import { creerRole, getAllRole, modifierRole, supprimerRole } from "@/app/api/settings/query";
    ...
    <>
      <Drawer anchor='right' open={isDrawerOpen} onClose={closeDrawer}>
          ...
          <div className="flex w-full max-w-xl flex-col items-center border bg-white p-10 text-left">
              <h2 className="mb-8 text-2xl font-bold">Ajouter un Nouveau Role</h2>
              <form className="w-full" onSubmit={handleSubmit}>     
                      ...
                      <div className="space-y-2">
                        ...
                        {paths.map((path, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            ...
                            />
                            <label htmlFor={`checkbox-${path}`} className="text-sm font-medium">
                              {path}
                            </label>
                          </div>
                        ))}
                ...
                <div className="ml-auto pt-8 w-full items-center justify-center flex font-medium">
                  <Button type="submit"  className="w-fit h-10 font-bold">Enregistrer</Button>
                </div>
              </form>
            </div>
             ) : (   
            ...
            <Button onClick={closeDrawer} className='font-bold bg-white hover:text-slate-900 hover:bg-slate-100 text-slate-600'>
                 <X />
                 Annuler
              </Button>
            </div>
      ...
      <div className="hidden flex-col max-h-[90vh] overflow-y-auto  md:flex">
        ...
        <div className="flex-1 md:space-y-4 space-y-4 p-8 pt-6">
          <div className="flex items-center justify-between  pb-2">
            <h2 className="text-4xl flex  items-center justify-center gap-2 font-bold tracking-tight"> <Settings/> Paramètres</h2>
              ...
              <div className="grid gap-4 py-4 md:grid-cols-2 lg:grid-cols-4">
                {roles.map((role)=>(
                    ...
                    <CardContent className="h-80">
                        <div className="py-4">
                          <div className="py-4">
                          <div className="font-bold text-xl">{role.nom}</div>
                          </div>
                          {Object.entries(role.access_groups).map(([key, value]) => (
                              <h3 key={key}>
                                - {key}
                              </h3>
                            ))}
                    ...
                    <CardFooter className="h-12 justify-between">
                        ...
                        <Button onClick={() => handleDeleteClick(role)} className='font-bold gap-1 text-sm bg-red-700 hover:bg-red-800'>
                            <Trash2 />
                            Supprimer
                          </Button>
                          <Button onClick={()=>handleRoleClick(role)} className='font-bold gap-2'>
                              <UserRoundPen />
                              Modifier
                            </Button>
                        </div></>
