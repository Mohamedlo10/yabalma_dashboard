...
import { MapPin, Phone, UserRoundSearch } from "lucide-react";
  ...
  return (
    <>
      ...
      <Drawer anchor="right" open={isDrawerOpen} onClose={closeDrawer}>
        <div className="p-4 h-full items-center justify-center flex w-[50vw] lg:w-[34vw]">
          {selectedCommande && (
            <div className="flex flex-1 flex-col h-full overflow-y-auto items-center justify-center">
              <div className="flex w-full max-w-xl flex-col items-center justify-center bg-slate-50 p-1 text-left">
                <div className="flex flex-row gap-2 w-full pb-4">
                  <div className="flex flex-col w-full items-center gap-2 ">
                    <div className="mb-2 text-sm font-bold leading-none">
                      Client
                    </div>
                    <Avatar className="hidden h-24 w-24  sm:flex">
                      <AvatarImage
                        src={`${selectedCommande.client?.img_url}`}
                    ...
                    <div className="grid gap-1">
                      <p className="text-sm font-bold leading-none text-red-700">
                  ...
                  <div className="flex flex-col w-full items-center gap-2 ">
                    ...
                    <Avatar className="hidden h-24 w-24  sm:flex">
                      <AvatarImage
                        src={`${selectedCommande.annonce?.client?.img_url}`}
                        className="rounded-full object-cover w-full h-full"
                        alt="Avatar"
                      />
                      <AvatarFallback>client</AvatarFallback>
                    ...
                    <div className="grid gap-1">
                      <p className="text-sm font-bold leading-none text-red-700">
                        {selectedCommande.annonce?.client?.prenom}{" "}
                        {selectedCommande.annonce?.client?.nom}
                      </p>
                    </div>
                ...
                <div className="pt-8 px-2 lg:px-2 xl:px-2 w-full grid items-center gap-0 justify-center grid-cols-2">
                  ...
                  <div className="text-sm grid grid-cols-2 col-span-2 gap-auto items-center justify-center font-bold  w-full mt-4">
                    ...
                    <div className="flex items-end w-full justify-end">
                      {format(
                        new Date(selectedCommande.created_at),
                        "dd MMMM yyyy",
                        { locale: fr }
                      )}
                      {` à ${format(
                  ...
                  <div className="flex flex-col w-full col-span-2 gap-4 ">
                    {selectedCommande.detail_commande.first_name && (
                      <div className="text-sm grid grid-cols-2 col-span-2 gap-auto items-center justify-center font-bold  w-full mt-4">
                        <div className="line-clamp-2 flex flex-row p-1 gap-4 font-bold text-sm text-muted-foreground">
                          <UserRoundSearch /> Destinataire:{" "}
                        </div>
                        <div className="flex items-end w-full justify-end">
                          {selectedCommande.detail_commande.first_name}{" "}
                          {selectedCommande.detail_commande.last_name}
                        </div>
                      </div>
                    )}

                    {selectedCommande.detail_commande.phone_number && (
                      <div className="text-sm grid grid-cols-2 gap-auto items-center justify-center font-bold  w-full ">
                        <div className="line-clamp-2 flex flex-row gap-4  p-1 font-bold text-sm text-muted-foreground">
                          <Phone /> Telephone:
                        </div>
                        <div className="flex items-end w-full justify-end">
                          {selectedCommande.detail_commande.phone_number}
                    ...
                    {selectedCommande.detail_commande.location && (
                      <div className="text-sm grid grid-cols-2 gap-auto items-center justify-center font-bold  w-full ">
                        <div className="line-clamp-2 flex flex-row gap-4  p-1 font-bold text-sm text-muted-foreground">
                          <MapPin /> lieu de livraison:
                        </div>
                        <div className="flex items-end w-full justify-end">
                          {selectedCommande.detail_commande.location}{" "}
                          {selectedCommande.detail_commande.code_postal}
                        </div>
                      </div>
                    )}
                  </div>
