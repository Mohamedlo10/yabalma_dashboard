...
export function AnnonceData({
  ...
  const annoncesWithFullNames = annonces.map((annonce) => ({
    ...annonce,
    trajet:`${annonce.source || ""}${annonce.destination || ""} ${annonce.destination || ""}${annonce.source || ""}`.toLowerCase(),
    client: annonce.client
      ? {
          ...annonce.client,
          id: annonce.client.id ?? null, 
          fullName: `${annonce.client.nom || ""}${annonce.client.prenom || ""} ${annonce.client.prenom || ""}${annonce.client.nom || ""}`.toLowerCase(),
        }
      : undefined,
  }));
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <ResizablePanel defaultSize={defaultLayout[1]} minSize={35} maxSize={40}>
          <Tabs defaultValue="all">
            ...
            <TabsContent value="is_boost" className="m-0">
              <AnnonceList items={filteredByDate.filter(item => item.is_boost == true)}/>
            </TabsContent>
            <TabsContent value="Validé" className="m-0">
              <AnnonceList items={filteredByDate.filter(item => item.statut == "Validé")}/>
            </TabsContent>
            <TabsContent value="En_attente" className="m-0">
              <AnnonceList items={filteredByDate.filter(item => item.statut == "En attente")}/>
            </TabsContent>
          </Tabs>
        </ResizablePanel>
        <ResizableHandle withHandle />
        <ResizablePanel defaultSize={defaultLayout[2]} minSize={35}  maxSize={40}>
          <AnnonceDisplay
            annonce={filteredByDate.find((item) => item.id === mail.selected) || null}
          />
        </ResizablePanel>
      </ResizablePanelGroup>
    </TooltipProvider>
