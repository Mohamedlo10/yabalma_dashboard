...
export const creerClient = async (clientData: Record<string, any>) => {
  ...
  try {
    const { data, error } = await supabase.from("client").insert([clientData]); // Insère l'objet client complet
...
export const getclientPays = async () => {
  ...
  try {
    const { data, error } = await supabase.rpc("client_count_by_country");
...
export const geTopClient = async () => {
  ...
  try {
    const { data, error } = await supabase.rpc("get_top_client");

    if (error)
      throw new Error("Erreur lors de la récupération du GP le plus ancien.");
